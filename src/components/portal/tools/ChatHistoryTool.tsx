...
export const ChatHistoryTool: React.FC = () => {
  ...
  }, [user, initialized, loadChatHistory]);
  
  // Process data whenever stored chat history changes - using useMemo to avoid excessive calculations
  const processedData = useMemo(() => {
    if (!storedChatHistory || !Array.isArray(storedChatHistory)) {
      return {
        assistantMessages: [],
        stats: {
          BULLISH: 0,
  ...
  // Update state based on processed data, but only when it changes
  useEffect(() => {
    setRecentMessages(processedData.assistantMessages);
    setSentimentStats(processedData.stats);
    setChatActivity(processedData.activity);
  }, [processedData]);
  ...
  // Calculate stats - derived from sentimentStats
